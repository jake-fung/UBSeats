  ...
  const renderMetricIcons = () => (
    <div className="flex space-x-3 text-gray-600">
      <div className="flex items-center">
        {spot.noise == 1 ? <Volume className="h-4 w-4 mr-1" /> :
            spot.noise == 2 ? <Volume1 className="h-4 w-4 mr-1" /> :
                spot.noise == 3 ? <Volume1 className="h-4 w-4 mr-1" /> :
                    spot.noise == 4 ? <Volume2 className="h-4 w-4 mr-1" /> :
      ...
      <div className={cn("p-4", featured ? "md:w-1/2 md:p-6" : "")}>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center text-sm text-gray-500">
            ...
            <span>{formatTime(spot.hours.open)} – {formatTime(spot.hours.close)}</span>
          </div>
        </div>

        <div className="flex justify-between">
          <h3 className={cn("mt-2 font-semibold text-gray-900 group-hover:text-primary transition-colors", featured ? "text-xl mb-2" : "text-lg mb-1")}>
            {spot.name}
          </h3>
        <span className={cn(
