...
test('matches an existing room by source_key', () => {
  const existing: ExistingRoom[] = [
    { uuid: 'r1', building_uuid: 'b-buch', room_name: 'BUCH – Room A101', source_key: 'BUCH-A101' },
  ];
...
test('skips rooms that exist as manual rows (no source_key) with the same trailing room token', () => {
  ...
  const plan = planRoomSync(
    [scraped({ source_key: 'IBLC-182', bldg_code: 'IBLC', room_number: '182' })],
    existing,
    buildings,
  );
...
test('throws when a bldg_code is missing from the buildings table', () => {
  assert.throws(
    () => planRoomSync([scraped({ bldg_code: 'NOPE', source_key: 'NOPE-1', room_number: '1' })], [], buildings),
    /NOPE/,
  );
});
